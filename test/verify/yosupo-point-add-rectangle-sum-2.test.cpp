...
int main() {
  ...
  for (int i = 0; i < Q; i++) {
    if (A[i] == 0) {
      ...
      B[i] = lower_bound(begin(ps), end(ps), make_pair(B[i], -inf)) -
          begin(ps);
      D[i] = lower_bound(begin(ps), end(ps), make_pair(D[i], -inf)) -
          begin(ps);
    }
  }
  ...
  auto bit =
      get_abstract_2d_binary_indexed_tree_compressed(H, f, 0LL);
  for (int i = 0; i < N; i++) {
    bit.apply(X[i], W[i]);
  }
  for (int i = 0; i < Q; i++) {
